...
Consigliere
Consort
Random Mafia`.toUpperCase().split('\n')
...
function doStuff(report, reportId) {
  ...
  /*
  for (let name in players) {
    if (mafiaRolelist.includes(players[name].role.toUpperCase().replace(/[0-9]/g, ''))) {
      console.log(`${players[name].role}`.red)
    } else {
      console.log(`${players[name].role}`.white)
    }
  }
  console.log('-----' + reportId + '------'.yellow)
  */
  ...
  for (let name in players) {
    ...
    if (player.killed.length) {
      if (player.killed[0].split('.')[0] == 'N1') { continue }
    }
    chat[players[name].role] = []
  }
  ...
  for (let i = 0; i < entries.length; i++) {
    ...
    if (entry.type == 'SYSTEM') {
      if (entry.text == 'Defense') { trial = true }
    }
    if (entry.type == 'VOTE') { trial = false }
    if (entry.type == 'DAY CHAT' && entry.time.split('.')[0] == 'D2' && !trial) {
      if (chat[players[entry.author].role]) {
        chat[players[entry.author].role].push(entry.text)
        //console.log('[' + players[entry.author].role + ']: ' + entry.text)
      }
    }
  }

  for (let role in chat) {
    ROLE = role.replace(/[0-9]/g, '')
    OUTPUT[ROLE] = OUTPUT[ROLE] || {}
    OUTPUT[ROLE].chat = OUTPUT[ROLE].chat || 0
    OUTPUT[ROLE].chat += chat[role].length
    OUTPUT[ROLE].matches = ++OUTPUT[ROLE].matches || 1
    OUTPUT[ROLE].avg = parseFloat((OUTPUT[ROLE].chat / OUTPUT[ROLE].matches).toFixed(2))
  }
